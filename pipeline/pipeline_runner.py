...
def _hash_password_for_pipeline(password: str) -> str:
    """PBKDF2-SHA256 해싱. 반환값: 'salt_hex:hash_hex' (deps.py 동일 로직)"""
    import os
    salt = os.urandom(16)
...
def _ensure_buyer(conn, buyer_name: str = "CLI_테스트_바이어") -> str:
    ...
    pw_hash = _hash_password_for_pipeline("test1234")
    row = db.execute_returning_with_conn(
        ...
        (buyer_name, "cli_pipeline", "cli_pipeline@imma.local", pw_hash),
    )
    return str(row[0])
